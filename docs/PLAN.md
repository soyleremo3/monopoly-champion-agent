...
- **The learned-value-probe direction is KILLed (`020`→`021`→`022`) — but
  scoped to the current broken `POLICY_ONLY` distribution, not a general
  claim.** `020` found some learnable full-horizon signal over uniform;
  `021` gave an unbiased TEST read showing the learned `ValueProbe` losing
  to a probabilistic net-worth-leader baseline on every metric
  ...
  none. See `docs/DECISIONS.md`'s "Close 021" entry. **Re-scope (`023`
  correction, see `docs/DECISIONS.md`'s "later still x7" entry): all
  `020`-`022` self-play games were generated under the same broken,
  no-buy/no-accept `POLICY_ONLY` policy `023` found (see next bullet) - the
  KILL holds for that specific distribution, not yet confirmed for a
  future distribution generated by a policy that actually buys property
  and negotiates trades.** **Current next milestone: the decision/policy
  win-rate phase** — this project's actual competition objective (see the
  Competition strategy section above); learned-value work is not resumed
  without a new proposed experiment and decision log giving a reason to
  expect it'll help this time.
- **`023`: bootstrap mismatch CONFIRMED — current `POLICY_ONLY` checkpoint
  KILLed as a strength candidate.** `baseline_pretraining.pt`'s actor is
  copied in full from a hybrid-PPO checkpoint whose `BUY_PROPERTY`/
  `ACCEPT_TRADE` action-head rows never received a PPO gradient update
  (verified statically against the pinned reference). Under plain
  `POLICY_ONLY` inference this checkpoint **never once** chooses
  `BUY_PROPERTY` or `ACCEPT_TRADE` (0/2,616 and 0/5,557 opportunities in
  `023`) - a systematic "never buy property, never accept a trade" defect.
  A diagnostic-only `HYBRID_COMPAT` policy that restores the reference's
  fixed rules on top of `POLICY_ONLY` (upper-bound measurement, **not** a
  submission candidate) won +21.25 points of win rate over plain
  `POLICY_ONLY` (46.25% vs. 25.0%, both PRIMARY cluster-aware stats
  excluding zero) - but that number was measured against three equally
  broken `POLICY_ONLY` peers, so it is **not** a general strength claim.
  No plain-`POLICY_ONLY` checkpoint should be proposed as a strength/
  submission candidate until the `BUY_PROPERTY`/`ACCEPT_TRADE`
  representation is actually fixed (retraining or another principled
  correction - not yet started). `baseline_pretraining.pt`'s PPO bootstrap
  is this project's own 1-game/598-step local stand-in (experiment `007`),
  **not** the upstream reference's documented "2000-game" artifact
  (SHA-256 mismatch confirmed in `023`) - the filename does not describe
  what this project actually trained on. See `docs/EXPERIMENTS.md`'s `023`
  entry and `docs/DECISIONS.md`'s "later still x7" correction entry.
...
12. ✅ Value decision audit (`022`) — post-hoc segment search across round
    ...
    decision/policy win-rate phase. Scope correction in `023`: this KILL
    holds for the broken no-buy/no-accept `POLICY_ONLY` distribution
    those games were generated under, not yet confirmed general.
13. ✅ Hybrid-PPO bootstrap isolation audit (`023`) — confirmed the
    `BUY_PROPERTY`/`ACCEPT_TRADE` bootstrap mismatch causes a large,
    statistically significant strength loss under `POLICY_ONLY`
    (+21.25 points win rate recovered by a diagnostic-only
    `HYBRID_COMPAT` upper-bound, against crippled `POLICY_ONLY` peers).
    Current `POLICY_ONLY` checkpoint **KILLed as a strength candidate**
    until this is fixed. Decomposition of how much each of `BUY_PROPERTY`
    vs. `ACCEPT_TRADE` contributes, and whether the effect holds against
    repaired (non-crippled) peers, is `024`, in progress.
